...
const Analyze = ({ onComplete, onBack, authHeaders = {} }) => {
  ...
  const validateForm = () => {
    ...
    else if (form.allergyMode === '있음' && !form.allergies.trim()) errors.allergies = true;
  ...
  React.useEffect(() => {
    ...
    fetch('/api/analyze', { method: 'POST', headers: authHeaders, body: fd })
      .then(r => {
              ...
              <div className="form-grid">
                  ...
                  <div className="fields-2">
                    ...
                    <div className="field">
                      <label className="field-label"
                        ...
                        {formErrors.allergyMode && <span style={{fontSize:11, marginLeft:6}}>선택해주세요</span>}
                        {formErrors.allergies   && <span style={{fontSize:11, marginLeft:6}}>성분명을 입력해주세요</span>}
                      </label>
                      <div className="chip-group" style={{marginBottom: form.allergyMode === '있음' ? 8 : 0}}>
                        {['없음', '있음'].map(t => (
                          <div key={t}
                            ...
                            onClick={() => {
                              setForm(f => ({
                                ...f,
                                allergyMode: t,
                                allergies: t === '없음' ? '없음' : (f.allergies === '없음' ? '' : f.allergies),
                              }));
                      ...
                      {form.allergyMode === '있음' && (
                        <input className="input"
                          placeholder="예: 향료, 에센셜오일, 알코올, 파라벤"
                          style={{borderColor: formErrors.allergies ? 'var(--warn)' : ''}}
                          value={form.allergies === '없음' ? '' : form.allergies}
                          onChange={e => {
                ...
                <section className="form-section">
                  ...
                  <div className="fields-3" style={{marginTop: 16}}>
                    <div className="field">
                      <label className="field-label">호르몬 · 스트레스 <span style={{fontSize:10.5, color:'var(--ink-faint)', fontWeight:400}}>(복수 선택)</span></label>
                      <div className="chip-group">
                        {['해당 없음','생리 전후 예민함','스트레스 심함','임신 중'].map(t => (
                          <div key={t} className={"chip " + (form.hormone.includes(t) ? 'selected' : '')}
                               onClick={() => toggle('hormone', t)}>{t}</div>
                        ))}
                      </div>
                    </div>
                    <div className="field">
                  ...
                  <div className="fields-2" style={{marginTop: 16}}>
                    ...
                    <div className="field">
                      <label className="field-label">식습관 <span style={{fontSize:10.5, color:'var(--ink-faint)', fontWeight:400}}>(복수 선택)</span></label>
                      <div className="field-hint" style={{marginBottom:6}}>야식, 정제탄수화물(빵/면/과자) 섭취 빈도</div>
                      <div className="chip-group">
                        {['해당 없음','야식 자주','정제탄수화물 자주'].map(t => (
                          <div key={t} className={"chip " + (form.diet.includes(t) ? 'selected' : '')}
                               onClick={() => toggle('diet', t)}>{t}</div>
                        ))}
                      </div>
                    </div>
                  </div>
