...
const Analyze = ({ onComplete, onBack, authHeaders = {}, skinProfile }) => {
  ...
  const [form, setForm] = React.useState({
    ...
    allergyMode: '', allergies: [], allergyOther: '',
    // 생활습관 (선택 - 정밀도 향상)
    drinking: '',
    smoking: '',
    cleansing: '',
  ...
  const loadPreviousForm = () => {
    ...
    fetch('/api/history/last_form', { headers: authHeaders })
      .then(res => {
          ...
          {step === 1 && (
            <>
              <div className="panel-head" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                >
                  <Icon name="refresh" size={14} /> 이전 기록 불러오기
                </button>
              </div>
                    ...
                    <div className="field">
                      ...
                      {form.allergyMode === '있음' && (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 8 }}>
                          <div className="chip-group">
                            {['알코올', '향료', '에센셜오일', '파라벤', '설페이트', '실리콘', '미네랄오일'].map(t => (
                              <div key={t}
                                className={"chip tone " + (form.allergies.includes(t) ? 'selected' : '')}
                                onClick={() => { toggle('allergies', t); setFormErrors(e => ({ ...e, allergies: false })); }}>{t}</div>
                            ))}
                          </div>
